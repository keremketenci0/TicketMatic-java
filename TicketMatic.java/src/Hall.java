...
public class Hall 
{
    ...
    public static Object[] generate_seats2() 
    {
        ...
        Random random = new Random();
        // determines the maximum number of occupied seats that can be
        int full_seat2 = random.nextInt((row * (2 * minrow + row - 1) / 2) - 2) + 1;
        // Marks the full_seat as "xx"
        ...
        String[][] seats2s = new String[seats2.length][seats2[0].length];
        for (int i = 0; i < seats2.length; i++) 
        {
            for (int j = 0; j < seats2[i].length; j++) 
            {
                seats2s[i][j] = seats2[i][j];
            }
        }
